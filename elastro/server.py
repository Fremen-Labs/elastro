    ...
    def _setup_routes(self) -> None:
        ...
        def get_clusters_health(
            ...
            for c in config.get("clusters", []):
                try:
                    ...
                    host = c["host"]
                    if not host.startswith("http://") and not host.startswith(
                        "https://"
                    ):
                        host = "http://" + host
                    ...
                    for idx in idx_res:
                        ...
                        try:
                            ...
                            if raw_size.endswith("pb"):
                                mult = 1024**5
                                val_str = raw_size[:-2]
                            elif raw_size.endswith("tb"):
                                mult = 1024**4
                                val_str = raw_size[:-2]
                            elif raw_size.endswith("gb"):
                                mult = 1024**3
                                val_str = raw_size[:-2]
                            elif raw_size.endswith("mb"):
                                mult = 1024**2
                                val_str = raw_size[:-2]
                            elif raw_size.endswith("kb"):
                                mult = 1024
                                val_str = raw_size[:-2]
                            elif raw_size.endswith("b"):
                                mult = 1
                                val_str = raw_size[:-1]
        ...
        if self.static_dir.exists():
            ...
            def serve_gui(full_path: str) -> HTMLResponse:
                ...
                return HTMLResponse(
                    "GUI not built. Run npm run build in packages/gui.", status_code=404
                )
