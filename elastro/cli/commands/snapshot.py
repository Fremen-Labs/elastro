...
def list_repositories(client):
    ...
    try:
        ...
        table.add_column(
            "Location", style="dim"
        )  # Assumes FS/S3 usually has 'location' or 'bucket'
...
def create_snapshot(client, repository, snapshot, indices, wait):
    ...
    manager = SnapshotManager(client)
    try:
        resp = manager.create_snapshot(
            repository, snapshot, indices=indices, wait_for_completion=wait
        )
        if wait:
            ...
            console.print(
                f"[bold green]Snapshot '{snapshot}' created (State: {state}).[/]"
            )
        else:
