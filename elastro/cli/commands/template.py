...
@click.option(
    "--type",
    type=click.Choice(["index", "component"]),
    default="index",
    help="Template type",
)
...
def wizard(client):
    ...
    if Confirm.ask("Configure Settings? (shards, replicas, codec)"):
        ...
        if Confirm.ask("Add Codec (compression)?"):
            codec = Prompt.ask(
                "Codec",
                choices=["default", "best_compression"],
    ...
    if Confirm.ask("Configure Mappings?"):
        while True:
            ...
            field_type = Prompt.ask(
                "Field Type",
                choices=["text", "keyword", "long", "integer", "date", "boolean", "ip"],
                default="keyword",
            )
    ...
    # Preview
    console.print(
        Panel(json.dumps(body, indent=2), title="Preview", border_style="green")
    )
    ...
    if Confirm.ask("Deploy to Cluster?"):
        try:
            if manager.create(name, body, template_type=template_type):
                console.print(
                    f"[bold green]Success![/] {template_type} template '{name}' created."
                )
            else:
