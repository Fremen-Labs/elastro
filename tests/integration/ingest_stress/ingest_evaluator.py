...
ARTIFACTS = Path("/Users/jonathandoughty/.gemini/antigravity/brain/e8ee424a-0e30-4543-a356-0ca3e27add02/artifacts")
...
def generate_data():
    ...
    # Complex/Dirty JSON
    complex_json = [
        {"id": 1, "name": "Alice", "settings": {"theme": "dark", "notifications": True}, "roles": ["admin", "user"]},
    ...
    with open(SCRATCH / "complex_dirty.ndjson", "w") as f:
        ...
        f.write(json.dumps({"id": 3, "name": "Charlie", "tags": "not_an_array_anymore"}) + "\n")
...
def run_tests():
    ...
    files_to_test = [
        "simple_clean.csv", "complex_dirty.csv",
        "simple_clean.json", "complex_dirty.json",
        "simple_clean.ndjson", "complex_dirty.ndjson",
        "simple_clean.sql", "complex_dirty.sql"
    ]
    ...
    for filename in files_to_test:
        ...
        cmd_import = [BIN, "ingest", "import", str(filepath), "--index", idx_name, "--dlq", str(dlq_path)]
        if fmt == "sql":
